...
app.post('/register', async (req, res) => {
    try {
        ...
        const existingUser = await client.db("2048").collection("user").findOne({ email });
        ...
        await client.db("2048").collection("user").insertOne({
            username,
            email,
...
app.post('/login', async (req, res) => {
   try {
       ...
       const user = await client.db("2048").collection("user").findOne({ email });
...
app.post('/saveScore', verifyToken, async (req, res) => {
    try {
        ...
        const user = await client.db("2048").collection("user").findOne({ _id: new ObjectId(req.userId) });

        if (user) {
            if (score > user.score) {
                const result = await client.db("2048").collection("user").updateOne(
...
app.get('/leaderboard', async (req, res) => {
    try {
        const leaderboard = await client.db("2048").collection("user")
            .find({}, { sort: { score: -1 } })
            .toArray();
